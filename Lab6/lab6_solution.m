...
[centers_outer, radii_outer, metric_outer] = imfindcircles(don_gray,[40 70], 'ObjectPolarity','dark', 'Sensitivity', 0.98, 'EdgeThreshold', 0.2);
[centers_inner, radii_inner, metric_inner] = imfindcircles(don_gray,[5 35], 'ObjectPolarity','dark', 'Sensitivity', 0.8, 'EdgeThreshold', 0.05);
% Display the outer circles superiposed on the image
close all
figure
imshow(don)
hold on
viscircles(centers_outer, radii_outer,'EdgeColor','green'); %roughly, we get 8 outer donut rings detected and..
viscircles(centers_inner, radii_inner,'EdgeColor','blue'); %we get three of the inner circles detected
...
%%SOLUTION
% Load the mask and masked image exported from the Image Segmenter App:

% Fusing, blending techniques might be tempting, but they don't achieve the
% pasting, since there is transparaceny left. Also there's no freedom wiht
% regards to the position where we want to place the target. 
pm = imfuse(scene, maskedImage_lw, 'Scaling', 'joint');
figure, imshow(pm)
figure, imshowpair(scene, maskedImage_lw, 'blend')

% A posible solution is the following:
% Use imcrop/imtool to get an approximation XY location in the scene image, where we want to place the
% lady in white;  
imtool(scene)
% We might not get the right locations from the first time, so we can 
% slightly increase or decrease the value
x_location = 2090 
y_location = 710
nrrows = size(mask_lw,1);
nrcols =size(mask_lw,2);
sceneroi = [x_location y_location  nrcols-1 nrrows-1 ];

% Crop from the scene image a patch of the same size as the image (painting) we want to
% paste. This way we can use the same mask for indexing.
roi = imcrop(scene, sceneroi);
% Use the mask to index both the scene ROI and the target to be pasted.
% This way only the pixels that correspond to white in the mask are
% replaced in the scene ROI.
m = repmat(mask_lw,[1,1,3]); % we need to replicate the mask; one mask for each color channel.
roi(m) = maskedImage_lw(m);
figure, imshow(roi), title('Patch from the scene with replacement')

% Now go back to the original image and replace the ROI with the modified
% patch;
resultimage = scene;
resultimage(y_location:y_location+nrrows-1, x_location:x_location+nrcols-1,:) = roi;
figure, imshow(resultimage), title('Lady-in-white reached Trolltunga!');
